...
Imports MMLCHelper.ApplicationMon_Student.IniFile.IniSection
    ...
    Public Class IniFile
        ...
        Public Function GetKeyValue(ByVal sSection As String, ByVal sKey As String) As String
            ...
            If (Not section Is Nothing) Then
                Dim key As IniKey = section.GetKey(sKey)
        ...
        Public Sub Load(ByVal sFileName As String, Optional ByVal bMerge As Boolean = False)
            ...
            If Not File.Exists(sFileName) Then
                Interaction.MsgBox(("File Not Found:" & Environment.NewLine & sFileName), MsgBoxStyle.ApplicationModal, Nothing)
                Application.Exit()
            Else
                Dim reader As New StreamReader(sFileName)
                Dim regex As New Regex("^([\s]*#.*)", (RegexOptions.Singleline Or RegexOptions.IgnoreCase))
                Dim regex3 As New Regex("^[\s]*\[[\s]*([^\[\s].*[^\s\]])[\s]*\][\s]*$", (RegexOptions.Singleline Or RegexOptions.IgnoreCase))
                Dim regex2 As New Regex("^\s*([^=\s]*)[^=]*=(.*)", (RegexOptions.Singleline Or RegexOptions.IgnoreCase))
                Do While Not reader.EndOfStream
                    ...
                    If (input <> String.Empty) Then
                        ...
                        If regex.Match(input).Success Then
                            ...
                            Trace.WriteLine(String.Format("Skipping Comment: {0}", match.Groups.Item(0).Value))
                        Else
                            If regex3.Match(input).Success Then
                                match = regex3.Match(input)
                                Trace.WriteLine(String.Format("Adding section [{0}]", match.Groups.Item(1).Value))
                            ...
                            If (regex2.Match(input).Success AndAlso (Not section Is Nothing)) Then
                                ...
                                Trace.WriteLine(String.Format("Adding Key [{0}]=[{1}]", match.Groups.Item(1).Value, match.Groups.Item(2).Value))
                                section.AddKey(match.Groups.Item(1).Value).Value = match.Groups.Item(2).Value
                                Continue Do
                            End If
                            If (Not section Is Nothing) Then
                ...
                Do While enumerator.MoveNext
                    ...
                    Try
                        ...
                        Do While enumerator2.MoveNext
                            Dim key As IniKey = DirectCast(enumerator2.Current, IniKey)
                            If (key.Value <> String.Empty) Then
                                Trace.WriteLine(String.Format("Writing Key: {0}={1}", key.Name, key.Value))
        ...
        Public Function SetKeyValue(ByVal sSection As String, ByVal sKey As String, ByVal sValue As String) As Boolean
            ...
            If (Not section Is Nothing) Then
                Dim key As IniKey = section.AddKey(sKey)
                If (Not key Is Nothing) Then
                    key.Value = sValue
                    Return True
                End If
