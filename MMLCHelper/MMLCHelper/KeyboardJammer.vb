...
Public Class KeyboardJammer
    ...
    Private Delegate Function HookCallback(ByVal nCode As Integer, ByVal wParam As Integer,
    ...
    Private Shared Function KeyBoardHookProc(ByVal nCode As Integer, ByVal wParam As Integer,
                                             ByVal lParam As IntPtr) As Integer
        'All keyboard events will be sent here.

        'Don't process just pass along.
        If nCode < 0 Then
            Return CallNextHookEx(HookId, nCode, CType(wParam, IntPtr), lParam)
        End If
        ...
        'flags instead
        Dim KeyboardSruct As KBDLLHOOKSTRUCT =
        ...
        ElseIf KeyboardSruct.vkCode = VK_LWinKey Or KeyboardSruct.vkCode = VK_RWinKey Then
            ...
            Return 1
        ElseIf KeyboardSruct.vkCode = vk_Del And My.Computer.Keyboard.CtrlKeyDown And _
    ...
    Public Shared Sub Jam()
        ...
        If HookId = 0 Then
            ...
            HookId = SetWindowsHookEx(Wh_Keyboard_LL, HookDelegate, Marshal.GetHINSTANCE(
